...
import {SidebarExampleDimmed} from './sidebar';
import categoryData from '../../assets/mocks/singleDay.json';

let store,wrapped;
beforeEach(()=>{
    wrapped = shallow(
                <SidebarExampleDimmed store={ store } dispatch = { random } storeSelectedData =  { initialState.stores.storeProfileDetails[0] }  />
              );
    wrapped.setProps({ dispatch: random, parentCallback: random , history: { push: random }, SingleDayCategories : categoryData.singleDay });
...
describe('Ordering SidebarExampleDimmed Component', () => {
  it('render SidebarExampleDimmed functions', () => {
